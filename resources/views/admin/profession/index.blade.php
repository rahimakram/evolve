            ...
            <div class="card">
                <div class="card-body">
                    <div class="row mb-2">
                        ...
                        <div class="col-sm-8">
                            <div class="text-sm-end">
                                ...
                                <a href="{{ route('admin.profession.add') }}"
                                    class="btn btn-success btn-rounded waves-effect waves-light addContact-modal mb-2"><i
                            ...
                            <tbody>
                                @forelse ($professions as $profession)
                                    <tr>
                                        ...
                                        <td>{{ $profession->profession_name }}</td> <!-- Trainer's name -->


                                        <td>
                                            @if ($profession->status == 'Y')
                                                <!-- Deactivate Button -->
                                                <button onclick="confirmDeactivate({{ $profession->id }})" type="button"
                                                    class="btn btn-sm btn-danger waves-effect waves-light">
                                                    <i class="bx bx-block font-size-16 align-middle me-2"></i> Deactivate
                                                </button>
                                                <form style="display:none"
                                                    id="deactivate_trainer_form_{{ $profession->id }}"
                                                    action="{{ route('admin.profession.deactivate', $profession->id) }}"
                                                    method="post">
                                                    @csrf
                                                </form>
                                            @else
                                                <!-- Activate Button -->
                                                <button onclick="confirmActivate({{ $profession->id }})" type="button"
                                                    class="btn btn-sm btn-success waves-effect waves-light">
                                                    <i class="bx bx-check-circle font-size-16 align-middle me-2"></i>
                                                    Activate
                                                </button>
                                                <form style="display:none" id="activate_trainer_form_{{ $profession->id }}"
                                                    action="{{ route('admin.profession.activate', $profession->id) }}"
                                                    method="post">
                                                    @csrf
                                                </form>
                                            @endif
                                        ...
                                        <td>
                                            ...
                                            <button type="button" onclick="confirmDelete({{ $profession->id }})"
                                                class="btn btn-sm btn-danger waves-effect waves-light">
                                                <i class="bx bx-trash font-size-16 align-middle me-2"></i> Delete
                                            </button>
                                            <form style="display:hidden" id="delete_trainer_form_{{ $profession->id }}"
                                                action="{{ route('admin.profession.delete', $profession->id) }}"
                                                method="post">
                                                @csrf
                                            </form>
                                        </td>
                    ...
                    <div class="d-flex justify-content-end mt-4">
                        {{ $professions->onEachSide(1)->links('pagination::bootstrap-5') }}
