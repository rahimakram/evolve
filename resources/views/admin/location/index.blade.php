                            ...
                            <tbody>
                                @forelse ($locations as $location)
                                    <tr>
                                        ...
                                        <td>
                                            @if (!empty($location->activity_names))
                                                {{ implode(', ', $location->activity_names->toArray()) }}
                                            @else
                                                N/A
                                            @endif
                                        </td>
                                        <td><img class="rounded-circle header-profile-user"
                                                src="{{ asset('storage/' . $location->logo) }}" alt="Header Avatar"></td>
                                        <td><img class="rounded-circle header-profile-user"
                                                src="{{ asset('storage/' . $location->image) }}" alt="Header Avatar"></td>


                                        <td>
                                            @if ($location->status == 'Y')
                                                <!-- Deactivate Button -->
                                                <button onclick="confirmDeactivate({{ $location->id }})" type="button"
                                                    class="btn btn-sm btn-danger waves-effect waves-light">
                                                    <i class="bx bx-block font-size-16 align-middle me-2"></i> Deactivate
                                                </button>
                                                <form style="display:none" id="deactivate_trainer_form_{{ $location->id }}"
                                                    action="{{ route('admin.location.deactivate', $location->id) }}"
                                                    method="post">
                                                    @csrf
                                                </form>
                                            @else
                                                <!-- Activate Button -->
                                                <button onclick="confirmActivate({{ $location->id }})" type="button"
                                                    class="btn btn-sm btn-success waves-effect waves-light">
                                                    <i class="bx bx-check-circle font-size-16 align-middle me-2"></i>
                                                    Activate
                                                </button>
                                                <form style="display:none" id="activate_trainer_form_{{ $location->id }}"
                                                    action="{{ route('admin.location.activate', $location->id) }}"
                                                    method="post">
                                                    @csrf
                                                </form>
                                            @endif
                                        ...
                                        <td>
                                            ...
                                            <button type="button" onclick="confirmDelete({{ $location->id }})"
                                                class="btn btn-sm btn-danger waves-effect waves-light">
                                                <i class="bx bx-trash font-size-16 align-middle me-2"></i> Delete
                                            </button>
                                            <form style="display:hidden" id="delete_trainer_form_{{ $location->id }}"
                                                action="{{ route('admin.location.delete', $location->id) }}"
                                                method="post">
                                                @csrf
                                            </form>
                                        </td>
