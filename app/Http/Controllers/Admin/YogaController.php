...
class YogaController extends Controller
{
    ...
    {
        // $yogas = Yoga::where('status', 'Y')->paginate(10);
        // dd($yoga);
        ...
        return view('admin.yoga.index', compact('yogas'));
    }
        ...
        if ($trainer_is_updated) {
            ...
            if ($trainerDetailsUpdated) {
                ...
                if ($request->hasFile('image')) {
                    ...
                    if ($trainerDetails->photo && \Storage::disk('public')->exists($trainerDetails->photo)) {
                        \Storage::disk('public')->delete($trainerDetails->photo);
                    }
                    $image = $request->file('image');
                    $imageName = $image->getClientOriginalName();
                    // $imageName = time() . '_' . $image->getClientOriginalName();
